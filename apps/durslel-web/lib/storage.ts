...
/**
 * Copy a job's artifacts to R2. The mp4 is what the user comes back for; scene.py and the log
 * ride along so a job stays diagnosable after the container is gone.
 *
 * `name` may contain slashes (`attempt-2/error.txt`) and is used as the key verbatim, which is
 * what lets a failed attempt keep its own folder under the job prefix instead of overwriting the
 * one before it.
 */
export async function publishJob(
  ...
  for (const { name, contentType } of files) {
    ...
    try {
      await put(jobKey(jobId, name), await readFile(local), contentType);
    } catch (e) {
