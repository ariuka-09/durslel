...
4. **In a `ThreeDScene`, EVERY piece of text must be fixed to the camera frame.** Pass each
...
5. **Fill the 16:9 frame.** Put the subject near the origin and scale it to roughly half the
...
6. **Total runtime under 60 seconds.** Keep `run_time` values modest and don't loop dozens of
   `self.play` calls.
7. End with `self.wait(1)` so the final frame holds.
